(function ($) {
    ...
    var navLink = 'http://www.brisnet.com/cgi-bin/briswatch.cgi/public/MarkHennig/HENNIG+MARK/9999/summary.html';

    $('#menu-item-20 a').attr('href', navLink);
...
*/
    ...
    $.ajax({
        type: 'GET',
        url: 'https://www.tlore.net/webservices/data.cfc?method=getRaceData&req=hennig8102&tid=112&date1=06/24/2018&date2='+ today,
        dataType: 'json',
        success: function (data) {
    ...
    $('th a').click(function() {
        ...
        var isNum= $table.find('tbody > tr:not(:first)').children('td').eq(column).hasClass('num');
        var rows = $table.find('tbody > tr:not(:first)').get();
    ...
    $.ajax({
        type: 'GET',
        //url: 'https://www.tlore.net/webservices/data.cfc?method=getWorkoutData&req=hennig8102&tid=112&date1=07/20/2018&date2='+ today +'&last=0',
        url: 'https://www.tlore.net/webservices/data.cfc?method=getWorkoutData&orderby=3&req=hennig8102&tid=112&date1=07/20/2018&date2='+ today +'&last=0',
        dataType: 'json',
        success: function(data){
            ...
            jQuery.each(arr, function (index, item) {
                var workoutArr = item.WorkoutDetail;
                   var date = item.WORKOUT_DATE.slice(0, 10);
                    var year = date.slice(2, 4);// year
                    var month = date.slice(5, 7);// month
                    var day = date.slice(8, 10);// day
                workoutContent = '<tr class="test-content">\n' +
                    '<td class="name-item">' + month + '/' + day  + '/' + year + '</td><td>\n';
                jQuery.each(workoutArr, function (index2, item2) {
                    workoutContent += '<div>' + item2.NAME + '</div>'
                });
                ...
                var emptyDivs = $('#workout tr td div').filter(function() {
                    return $.trim($(this).text()) === "";
                });
                emptyDivs.remove();
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            jQuery.each(arr, function (index, item) {
                ...
                var emptyDivs = $('#future-races tr td div').filter(function() {
                    return $.trim($(this).text()) === "";
                });
                emptyDivs.remove();
